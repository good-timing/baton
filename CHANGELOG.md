...
- **The same rewording was applied to the `overall_task` param and then reverted, because it was measured and lost.** The candidate ("the specific task the user is working on right now — not the overall theme of the conversation") was run against the shipped text on 2026-08-11: 20 paired live-agent sessions, scripted multi-turn conversations with known ground truth, both arms served from one build (`baton-internal/spikes/overall_task_a5/` §A5b). The two wordings produced identical grouping behaviour on 19 of 20 sessions; on the 20th the candidate relabelled *within* a single task and then returned to the first label (`A → B → A`), which a merge-only, adjacency-based consumer resolves as three tasks rather than one. No measured upside, one measured downside — so the param text is unchanged and this release carries no `overall_task` semantics change. **Do not re-apply the rewording without new evidence.**

- **Correspondingly, one earlier claim is withdrawn.** Previous notes said the shipped "broader task" phrasing elicits conversation-scoped umbrella labels. That still holds for the sessions it was measured on, but it did **not** reproduce in A5b: on five multi-task scripts the shipped text produced correct per-task labels every time. The difference is likely corpus — A5b's scripts announce their topic changes ("Different thing:", "New topic:") — so the umbrella question is open rather than settled, pending a cue-free corpus. What is unchanged is the *asymmetry* between the two surfaces on identical sessions: the injected param groups more reliably than the annotation field, which is why SPEC §11.5.2 and §13 tell consumers to treat annotation-sourced `workflow` as weaker evidence than `call_workflow`.
