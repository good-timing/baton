...
- **`correlation_mode` on every emitted event.** SPEC §11.4's example envelope has carried this field since it was written, §3.4 rung 5 sets it, and §11.5 gives the Console worker rules keyed on it — and `_EventEnvelope` had no such field, so no Baton producer had ever emitted one. Additive and non-nullable, defaulting to `"session-stitched"`; that is the value every SDK path emits today, because rung 5 — the only rung that yields `per-event` — is unbuilt in both adapters (design-note D2). **A constant still earns its place**, and this is the whole argument for shipping it now rather than with rung 5: without the field a consumer cannot tell *"every event has a different `session_id` because that is the declared mode"* from *"every event has a different `session_id` because the resolver is broken"* — and this very release's fastmcp-4 entry describes exactly that confusion from the other side, calling the broken per-request ids "indistinguishable on the wire from legitimate `per-event` correlation". They are now distinguishable. Asserted in `tests/functional/envelope_assertions.py` against all three capture paths at once, including the rule that one run declares exactly ONE mode — a producer mixing both within a session would tell the worker to group and not to group the same events. `baton-spec` updated in lockstep (`14a4d99`); the five existing conformance vectors validate unchanged. **Not included:** rung 5 itself, and the Console honouring the field — this is the producer half.
