...
- **The `workflow` annotation field and the `overall_task` param now ask for the user's *current task*, not "the broader task".** Measured 2026-08-10 against scripted multi-turn sessions with known ground truth (`baton-internal/spikes/overall_task_a5/`): the old "broader task ... change it only when the user starts a different task" phrasing elicits conversation-scoped umbrella labels — one label held across genuinely distinct requests — which silently merges tasks in any correlation rule keyed on the field. Both surfaces now ask for a short current-task label with an explicit repeat-verbatim-until-they-switch stability contract, and the annotation tool's `workflow` gains that contract for the first time (it previously had none). Text-only: same fields, same events, no consumer change. **Scope of the evidence, stated precisely:** the umbrella-label behaviour was measured on the OLD wording, on both surfaces; the new wording is not yet validated against live agents (the first attempts to test it ran against a stale build — see the methodology note in the spike). What *is* established under the old wording is a divergence between the two surfaces on identical sessions: the injected param produced per-task labels while the annotation field produced umbrella ones. That is why SPEC §11.5.2 and §13 now tell consumers to treat annotation-sourced `workflow` as weaker evidence than `call_workflow` — a rule that holds regardless of which wording ships.
