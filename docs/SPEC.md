...
- **0.8.10 / proxy 0.6.11 (2026-09-17)** — **BREAKING: the flat field `principal_id` becomes the object `principal`, carrying `id` + `source` + `form`; the scheme tag `v1:` is RETIRED and §11.4's pseudonym discriminator is REPLACED.** Carried by `baton-sdk` 0.8.10 and `baton-proxy` 0.6.11. A required-shape change on an existing field plus a value change and a replaced consumer rule, shipped as a patch because there are no customers on this format and pre-1.0 breakage rides a patch under this section's policy. ⚠ **`baton-extmcp` emits the object through this package's Emitter and is not tagged**, so its wire shape follows whichever `baton-proxy` its image resolves — pin it below 0.6.11 to hold the old shape.
