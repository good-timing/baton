...
When the client populates `_meta["io.baton/*"]` keys directly (e.g., via a vendor's MCP-client instructions), those take precedence over per-runtime adapters — `_meta["io.baton/agent_runtime"]` overrides the detected runtime. Two of the four keys above are read today: this one, and `_meta["io.baton/session_id"]`, which is rung 2 of §3.4's ladder and outranks the `mcp-session-id` header. `io.baton/mcp_transport` and `io.baton/vendor_app_version` are specified but not yet read by any adapter.

> **Corrected 2026-09-09.** This paragraph previously named a nested `_meta.baton.*` form, contradicting the key table two paragraphs above it — and the SDK's detector followed *this* line rather than the table, so a vendor asserting its runtime the way the table documents was silently ignored. The reverse-DNS form in the table is the correct one (it is what the MCP `_meta` convention produces); the nested form is gone from the SDK and was never sent by anything.
>
> **Sensor divergence, until baton-proxy and baton-ts follow.** Both mirror this heuristic and both still read only the nested form. So a client that sets `io.baton/agent_runtime` is recorded with its asserted value by this SDK and with a detected or default value by those two — and since they are separate sensors that can observe the same client, a query grouping on `agent_runtime` will split that client in two until they are updated. A deployment running more than one sensor SHOULD update them together.
