...
- **Unreleased (SDK-only, 2026-09-08b)** — **`correlation_mode` is now actually emitted.** The field has been in §11.4's own example envelope since it was written, §3.4 rung 5 sets it, and §11.5 gives the worker rules for it — and no producer had ever put it on an event. Additive, non-nullable, defaulting to `"session-stitched"`, which is what every SDK path emits today because rung 5 (the only thing that yields `per-event`) is unbuilt in both adapters. **Consumer consequence, and the reason a constant is still worth sending:** without this field a consumer cannot distinguish *"every event carries a different `session_id` because the producer declared per-event mode"* from *"every event carries a different `session_id` because the producer's resolver is broken"* — which is not hypothetical, since the 2026-09-04 entry above describes fastmcp 4's per-request ids as "indistinguishable on the wire from legitimate `per-event` correlation". They are now distinguishable: a producer declaring `session-stitched` while emitting an id that never repeats is broken, and a consumer may say so instead of guessing. Events from producers predating this field are unaffected — the schema default means absent reads as `session-stitched`, which is what they were. `baton-spec` carries the field as of `14a4d99`; all five existing conformance vectors validate unchanged. **Not shipped with it:** rung 5 itself, and the Console honouring the field. This entry is the wire half only.
