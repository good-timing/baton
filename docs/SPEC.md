...
- **Unreleased (SDK-only, 2026-08-10)** — no shape change; **semantics sharpened on the `workflow` annotation field only** (§11.5.2 table). It previously said "the broader task this call is part of" with no stability contract; it now asks for the user's **current-task** label plus an explicit repeat-verbatim-until-they-switch contract. Producers emit the same field in the same place; only the text agents read changed, so consumers need no change and old events stay valid. The same rewording was applied to the injected `overall_task` param and then **reverted**: measured head-to-head against the shipped text over 20 paired live-agent sessions from one build (2026-08-11), it produced identical grouping on 19 of 20 and, on the 20th, relabelled within a single task and returned to the first label (`A → B → A`) — which a merge-only, adjacency-based correlator resolves as three tasks instead of one. The param therefore keeps its shipped wording, and the two surfaces are intentionally worded differently. **Consumer guidance, unchanged and strengthened:** on identical scripted sessions the injected param groups more reliably than the annotation field — 9/10 vs 8/10 correct task counts on the single-task subset, with the one clean divergence going the param's way. Correlation SHOULD treat annotation-sourced `workflow` as weaker evidence than `call_workflow`, not stronger. **Withdrawn:** the earlier claim that the shipped param wording elicits conversation-scoped umbrella labels did not reproduce in the 2026-08-11 corpus (5/5 multi-task scripts correct); it is open, not established. Evidence: `baton-internal/spikes/overall_task_a5/` §A5b.
