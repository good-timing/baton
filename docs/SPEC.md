...
**`user_id` — the end-user actor (OPTIONAL, nullable).** Which person the vendor identifies as being behind this call, so the collector can answer "which *customer* hit this" and group by `(tenant_id, vendor_id, user_id)`. Absent whenever no identity was resolved, which is the common case and never an error.
...
Keep all of it apart from `agent_runtime`. `agent_runtime` is what the client *says* it is — self-asserted, never checked, and not a person at all. `user_id` names a person, attested or asserted. They answer different questions and they are trustworthy to different degrees.
...
**A consumer MUST NOT assume the value is a pseudonym.** In `"raw"` mode it is real end-user identity and carries whatever retention and residency obligations that implies.
