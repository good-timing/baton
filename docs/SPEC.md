...
- **Unreleased (SDK-only, 2026-08-06)** — no envelope/field changes; the SDK's injected schema param names changed from the single `baton_intent` to vendor-neutral `user_goal` + `expected_result`, matching baton-extmcp's naming (white-label rule — see `docs/design-notes/intent_param_injection.md` D1 for the superseded namespaced-collision rationale and its divergence note). `user_goal` still feeds `call_intent`/`intent_source`; `expected_result` is newly wired to the existing `AnnotationPayload.expected_outcome` field (previously reachable only via a real annotation-tool call, never via injection). baton-proxy is unaffected and still emits/expects `baton_intent` — the two surfaces intentionally diverge on the agent-facing param name until proxy is ported separately.
