...
| `workflow` | string \| null | yes (nullable) | Short label for the task the user is working on **at the time of this signal** — not the overall theme of the conversation (e.g., "morning meeting prep", "pre-outreach research"). Carries the same string-stability contract as the injected `overall_task` param (§11.4.2): repeated verbatim while one task continues, replaced when the user switches. Correlation keys on it (§11.5), so stability is load-bearing. Promoted from `context.workflow` after empirical validation across proactive annotations. Source: agent via annotation tool. May be null. **Measured caveat (2026-08-11):** agents supply conversation-scoped umbrella labels here far more often than they do on the injected param, and matched wording did not close the gap — treat annotation-sourced `workflow` as weaker evidence than `call_workflow`. |
...
- **Unreleased (SDK-only, 2026-08-11b)** — no wire/shape change; **default capture behavior changed**. New `VendorConfig.proactive_mode`, defaulting to `"off"`: the server instructions no longer ask the agent to file a proactive annotation before each tool call, because the injected params (§11.4.2) now carry `call_intent`/`call_expected`/`call_workflow` on every `tool_call_start` — the same three fields, no extra inference turn, and measurably better capture (params 1.000 coverage / per-task labels; annotation path 0.135 / umbrella labels). Reactive annotation is unchanged in both modes and remains the product signal; the SDK-synthesized proactive still fires, so `annotation` events with `intent_source="injected_param"` continue to appear and consumers see no field disappear. Producers on `proactive_mode="on"` emit exactly what they emit today. Two consumer-visible consequences: (1) agent-authored proactive annotations become rare rather than absent — the mode is instruction text, not enforcement — so a correlation rule keyed on annotation `workflow` can still be tripped by a single stray umbrella label, and SHOULD prefer `call_workflow`; (2) any consumer using proactive annotations as a *turn* boundary (§11.5 tier 2) loses roughly half its boundaries on this traffic and should derive units from `call_workflow` transitions instead. Evidence: `baton-internal/spikes/overall_task_a5/`.
- **Unreleased (SDK-only, 2026-08-11)** — no shape change; **semantics sharpened** on the `workflow` annotation field (§11.5.2 table) and on the injected `overall_task` param description. Both previously said "the broader task this call is part of", which measurement showed elicits *conversation-scoped umbrella labels* — one label held across genuinely distinct user requests, which silently merges tasks in any correlation rule keyed on it. Both now ask for the user's **current-task** label plus an explicit repeat-verbatim-until-they-switch stability contract. Producers emit the same field in the same place; only the text agents read changed, so consumers need no change and old events stay valid. **Measured asymmetry worth encoding in consumers:** matched wording did NOT equalize the two paths — on the same scripted sessions the injected param produced per-task labels while the annotation field still produced umbrella labels, repeatably. Position appears to beat wording: a param is filled in the context of one call, an annotation is authored while the agent is reasoning about the session. Correlation SHOULD therefore treat annotation-sourced `workflow` as weaker evidence than `call_workflow`, not stronger. Evidence: `baton-internal/spikes/overall_task_a5/`.
