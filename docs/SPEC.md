...
0. **Vendor-configured resolver hook** — `VendorConfig.resolve_session_id`, an optional SDK-side hook checked before every rung below. Lets a vendor with their own session/auth concept hand the SDK a real correlation key directly, bypassing MCP transport/meta entirely — the only mechanism that can work on new-spec (SEP-2567) or true-stateless streamable HTTP, where nothing below is observable by protocol design. A non-empty string return wins outright; `None`/empty or a raised exception falls through to the first live rung below (rungs 1-2 are retired, so in practice rung 3 or 4). Not part of the wire format — an SDK-local resolution strategy feeding the same `session_id` field every other rung populates. See `baton-sdk`'s `docs/design-notes/session_resolver_hook.md`.
...
5. **Per-event UUID** — when 0-4b all yield nothing. Sets `correlation_mode=per-event`. **Not implemented by any adapter yet**: both terminate instead on a process-wide id minted at install, which is stable but merges every client of a multi-user server. Tracked as design-note D2.
...
⚠ This block described `title` and `body` fields and a `200` until 2026-09-11.
...
`baton-sdk` shipped a `handle.escalate()` helper through 0.8.0. It was never
callable in practice: the key an installed SDK holds is a `write_events`
publishable key, which this endpoint refuses, so every call from a wrapped
server would have 403'd. Nothing in the SDK, its examples, `baton-proxy` or
`baton-ts` ever called it. The method, the Console URL and API key it read off
the sink, and its shared HTTP client are gone — **a `BatonHandle` now makes no
network calls of any kind.** A vendor tool that wants to file a ticket calls
the Console endpoint itself, with a key scoped to do it.
...
```json
{
  ...
  "sdk_version": "0.1.0",
  "agent_runtime": "claude-code",
  "user_id": "h1:9f2c...",                   // optional; the authenticated end user, hashed at the edge; see below
...
**`user_id` — the attested end-user actor (OPTIONAL, nullable).** The principal the vendor's own auth verified, so the collector can answer "which *customer* hit this" and group by `(tenant_id, vendor_id, user_id)`. Absent whenever no identity was resolved, which is the common case and never an error.

Keep it apart from `agent_runtime`. `agent_runtime` is what the client *says* it is — self-asserted, never checked. `user_id` is derived from a bearer token the vendor's verifier already validated. They answer different questions and they are trustworthy to different degrees.

| aspect | rule |
|---|---|
| source | the OIDC subject (`sub`) of the verified access token, keyed with its issuer (`iss`). **Never `client_id`**, which names the OAuth application and is identical for every user of one app. |
| transport | HTTP only. MCP auth is ASGI middleware, so a stdio deployment has no token and this field is always absent there. |
| derivation | `"hashed"` (default) emits `h1:<hex>` — HMAC-SHA256 over `(tenant_id, sub, iss)`, computed **in the producer's process**. `"raw"` emits `sub` verbatim. |
| `h1:` prefix | the scheme tag, and the key-rotation seam: rotating the secret cuts new hashes to a new tag while historical ones keep theirs. It also makes the two modes self-distinguishing on the wire — a value carrying no scheme prefix is a raw principal. |

**A consumer MUST NOT assume the value is a pseudonym.** In `"raw"` mode it is real end-user identity and carries whatever retention and residency obligations that implies. The `h1:` prefix is the discriminator; treat its absence as personal data.

**A hashed `user_id` is not reversible by the collector, by design.** The HMAC secret belongs to the producer and is never transmitted — if the collector held it, it could hash candidate identities and recover the column, which is what hashing at the edge exists to prevent. A hash is also stable only within one tenant and one key generation: the same person under two tenants, or across a key rotation, is two different values, and this is deliberate rather than a defect to correct downstream.
...
- **It says WHICH CALL, never WHO.** The principal is `user_id`, a different field on a different condition. Keep the two claims apart.
