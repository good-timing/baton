...
- `src/baton/` — the SDK package (Python). Core substrate at the top level (`sinks.py`, `events.py`, `scrub.py`, `_state.py`, `client.py`, `_dsn.py`, `_optout.py`). `_optout.py` is the off switch (`BATON_DISABLED`, and deliberately NOT `DO_NOT_TRACK` — an MCP client passes its server a 6-variable allowlist, so honouring that convention bought nothing and cost a red suite for anyone with it exported): checked FIRST at all five entry points, and "off" means install nothing AND never throw — a switch that can abort a vendor's boot is worse than no switch. `_dsn.py` is the only reader of the packed `dsn` a Console mints — the grammar's one implementation, shared by both adapters AND the library API so no door can resolve a customer's workspace differently from another, and the home of `VENDOR_ID_PATTERN` because a DSN's server segment IS a `vendor_id`. Integrations under `src/baton/integrations/*` — today: `mcp/` (adapts the official Anthropic `mcp` SDK's `mcp.server.fastmcp.FastMCP` via tool-handler wrapping) and `fastmcp/` (adapts the standalone `fastmcp` library's `fastmcp.FastMCP` via middleware). Both expose the same `install_baton`, `VendorConfig`, `BatonHandle` surface; vendors pick the adapter that matches their host library. Pattern: core SDK + opt-in integrations via pip extras.
