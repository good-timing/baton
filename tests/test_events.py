...
def _envelope() -> dict[str, Any]:
    """Common envelope fields a test event needs. event_id + spec_version +
    sdk_version + event_type are set by defaults on the concrete model."""
    return {
        "tenant_id": "ten_01H4F",
...
class TestToolCallStartEvent:
    ...
    def test_spec_version_default(self) -> None:
        event = ToolCallStartEvent(**_envelope(), payload=ToolCallStartPayload(tool_name="t"))
        assert event.spec_version == "0.2"
