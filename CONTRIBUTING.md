...
3. `docs/SKILLS_LIBRARY_API_DRAFT.md` — the library API surface alongside the MCP middleware.

## Spec-first, failing-test-first

For any non-trivial change:

1. **Spec.** If the change affects the wire format or public API contract, update `docs/SPEC.md` (and/or `docs/CHARTER.md` if it touches a load-bearing decision) first. For small internal changes, a comment in the PR is fine.
2. **Failing test.** Write a test against the fake-vendor fixture. Run it. Confirm RED.
...
- **Fake-vendor fixtures only.** The test suite uses synthetic vendor stubs. If a test imports a real vendor module, it's wrong (per CHARTER §4 rule 5).
...
- Changes that introduce stateful logic in the SDK beyond the bounded event buffer. Per CHARTER OD-7, the SDK is thin-emit-only; correlation, policy, and dispatch live in the Console worker.
