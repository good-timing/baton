...
string that compliant clients SHOULD surface to the calling LLM. v0.1 spike
validation: load-bearing in Claude Code + Cursor; ignored by Claude Desktop
and the Claude.ai web MCP connector (Anthropic's own docs say "only tool
calls are currently supported" on the consumer surfaces).

Two framing decisions are load-bearing per the spike findings:
  1. Top-level fields use MUST + (REQUIRED) markers — milder framing
     under-populates them (Round 4 over-correction).
  2. ``context`` is framed positively as "supplementary" — anti-duplication
     warnings backfire (same Round 4).

The packed annotation tool description (per Round 9 follow-up — see archived
v0.1 spike record) carries equivalent framing for runtimes that don't surface
``instructions``. Belt + suspenders.
"""
