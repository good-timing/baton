...
Per SPEC §11.2 SDK conformance + CHARTER OD-7 (thin-emit; never block
...
correlation, no detection, no policy (all worker-side per OD-7). The
...
class BatonMiddleware(Middleware):
    ...
    async def on_call_tool(
        ...
        runtime = (
            detect_agent_runtime(self._extract_request_meta(context))
