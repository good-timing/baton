...
Sync usage (drives the async ``EventEmitter`` via a background daemon thread
...
like Sentry):
    ...
    client = Client(
        api_key="bk_live_...",
        ingest_url="https://acme.console.example.com",
        vendor_id="acme",
        consent_token="...",
    )
    try:
...
Async usage (no thread bridge; directly drives the async ``EventEmitter``):
...
Config loading: explicit kwargs win; env-var fallback supported per phase 1
decisions (``BATON_API_KEY``, ``BATON_INGEST_URL``, ``BATON_VENDOR_ID``,
``BATON_CONSENT_TOKEN``). ``consent_token`` may be overridden per-trace.
"""
...
def _resolve_config_value(
    ...
    """Phase-1 decision: explicit kwargs win; env-var fallback supported.
...
class Trace:
    ...
    def __init__(
        ...
        *,
        ...
        # Per-trace consent_token overrides the Client-level default per phase-1
        # decision 3.
        self._consent_token: str = consent_token or client._consent_token
        # session_id per the SPEC §3.4 layered fallback. In library mode v0.1
        # we default to per-event (fresh UUID per trace); per-trace override
...
class Client:
    ...
    def __init__(
        ...
        **emitter_kwargs: Any,
        ...
        api_key_resolved = _resolve_config_value(
            api_key, "BATON_API_KEY", required=True, name="api_key"
        )
        ingest_url_resolved = _resolve_config_value(
            ingest_url, "BATON_INGEST_URL", required=True, name="ingest_url"
        )
        ...
        assert api_key_resolved is not None
        assert ingest_url_resolved is not None
        ...
        # Sync mode uses a background thread + persistent loop bridge.
        self._bridge = _SyncBridge()
        self._emitter: EventEmitter = self._bridge.run(
            self._init_emitter(
                console_url=ingest_url_resolved,
                api_key=api_key_resolved,
                **emitter_kwargs,
            )
        )

        # Per-session sequence counters. Library mode in v0.1 = per-event mode
        # (each Trace generates a fresh session_id), so each session_id has
        # exactly one trace's worth of events. Counter still maintained for
        # consistency with SPEC §11.4 (monotonic sequence_number per session).
        self._seq_counters: dict[str, int] = {}
        self._closed = False

    @staticmethod
    async def _init_emitter(**kwargs: Any) -> EventEmitter:
        """Constructed inside the bridge loop so the emitter's async primitives
        (lock, drain task) bind to the bridge loop."""
    ...
    def flush(self) -> None:
        ...
        if self._closed:
            return
        self._bridge.run(self._emitter.flush())

    def close(self) -> None:
        """Flush + close the emitter + stop the bridge thread."""
        if self._closed:
            return
        try:
            self._bridge.run(self._emitter.aclose())
        finally:
            self._closed = True
            self._bridge.stop()
    ...
    def _emit_sync(self, event: Any) -> None:
        self._bridge.run(self._emitter.emit(event))
