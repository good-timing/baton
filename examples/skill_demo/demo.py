"""Worked example: Baton library API instrumenting a Skill-pattern code path.
...
the reactive friction signal that becomes a collector-side "ticket").

The vendor SDK is stubbed (``fake_vendor``) so this runs offline. The bug we
reproduce is the kind of capability-mismatch failure common across inference
APIs: a small model returns a 400 ("Grammar must have a 'properties' field")
when called with ``response_format={"type": "json_schema", ...}`` even though
the vendor's docs and Skill imply json_schema is broadly supported.
...
def main() -> None:
    # Init the Baton client. Library API takes any Sink — here we ship events
    # over HTTP to the local capture server (see local_ingest.py). Swap in
    # StdoutSink() for a zero-dependency dev mode.
