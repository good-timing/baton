"""Stubbed vendor SDK shape (OpenAI-compatible chat completions).

Drop-in replacement for a typical vendor SDK with
``client.chat.completions.create(...)`` (the shape used by OpenAI, Together,
Anthropic-compat servers, Groq, Mistral, etc.). Reproduces a documented
capability-mismatch failure mode: certain models return a 400 with the
message::

    BadRequestError: Grammar must have a 'properties' field

when the caller passes ``response_format={"type": "json_schema", ...}`` even
though the docs imply json_schema is broadly supported. The exact wording
mirrors real-world inference-API errors that surface across multiple vendors.
