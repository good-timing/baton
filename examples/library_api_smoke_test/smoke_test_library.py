...
What this does NOT validate (out-of-scope per PLAN.md):

- Skill-author adherence in production (Skill-design, not engineering)
- Session correlation under streamable HTTP (per-event mode default in v0.1)
- consent_token scaling beyond v0.1 single-static-UUID model
...
def assert_events(events: list[dict[str, Any]]) -> None:
    ...
    for e in events:
        ...
        assert e["spec_version"] == "0.2", f"unexpected spec_version: {e['spec_version']}"
