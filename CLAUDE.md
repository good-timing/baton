...
- `src/baton/integrations/standalone/` — **Standalone `fastmcp` adapter** (targets `fastmcp.FastMCP` v2.x): `install.py` (entrypoint + `VendorConfig`), `middleware.py` (`BatonMiddleware` — uses fastmcp's native middleware chain), `annotation.py`, `instructions.py`.
- `src/baton/integrations/runtime_adapter.py` — **`_meta`-based agent-runtime detection, shared by BOTH adapters.** Lived under `standalone/` until 2026-09-09, which is why the official adapter never called it and shipped `agent_runtime: "unknown"` on every event. Call it on the RAW `_meta`, before the vendor's scrubber: the default scrubber is a no-op, so a post-scrub call passes here and fails only at a vendor. **There is no client override any more** — the `io.baton/*` keys were removed 2026-09-09, so every value this returns is an SDK-controlled constant, never client text, which is why nothing here is scrubbed or capped. The first tier to read a client-supplied value must bring its own.
